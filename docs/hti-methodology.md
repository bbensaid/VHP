...
The **Health Transformation Index (HTI)** is a composite metric that tracks the overall state of U.S. healthcare transformation across the five analytical pillars: Policy, Economics, Technology, Clinical, and Equity. It produces:

- A **national composite score** (0–100) updated quarterly
- **Five pillar sub-indexes** (0–100 each)
- **State Performance Index scores** for all 50 states across six dimensions
...
│
├── Technology Sub-Index      (20% weight)
...
| Technology | 20% | Enabler of all other transformation; high investment activity |
| Clinical | 20% | Ultimate purpose — actual care delivery improvement |
| Equity | 15% | Critical cross-cutting lens; historically underpowered in existing indexes |
...
```text
HTI = (Policy × 0.20) + (Economics × 0.25) + (Technology × 0.20) + (Clinical × 0.20) + (Equity × 0.15)
```
...
| --- | --- | --- |
| **Performance Score** | Composite | Weighted average of all five dimensions below |
...
- **Pillar Cards** — five cards showing each pillar sub-index with trend arrow and top-line summary
