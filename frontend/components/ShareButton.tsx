...
export default function ShareButton({ title, summary }: ShareButtonProps) {
  const handleShare = () => {
    ...
    window.open(linkedinUrl, 'linkedin-share', 'width=550,height=600');
  };

  return (
    <button
      onClick={handleShare}
      className="flex items-center gap-1.5 text-slate-400 hover:text-[#0077b5] transition-colors group"
