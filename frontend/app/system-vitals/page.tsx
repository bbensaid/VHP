...
// Used by distance-based algorithms. Symmetric — only upper triangle stored.
...
const TRANSFER_ALGOS: AlgoDef<TransferAlgoId>[] = [
  {
    ...
    description: "Balances bed availability, specialty coverage, hospital tier, and transfer infrastructure. Best general-purpose choice.",
  },
  {
    id: "specialty_first",
    label: "Specialty first",
    description: "Prioritizes specialty-matched facilities above all else. Best for complex or subspecialty cases where clinical expertise is the binding constraint.",
  },
  {
    id: "closest",
    label: "Closest available bed",
    description: "Ranks by estimated drive time from the sending hospital. Best for time-sensitive stable transfers where minutes matter.",
  },
  {
    id: "capacity_first",
    label: "Most available capacity",
    description: "Maximizes the open bed percentage in the requested unit. Best during system-wide surge when finding a reliable bed fast is the priority.",
  },
  {
    id: "conserve_tertiary",
    label: "Conserve tertiary capacity",
    description: "Prefers regional and critical access hospitals over tertiary centers. Best for step-down patients who don't require advanced subspecialty resources.",
  },
];
...
const REPAT_ALGOS: AlgoDef<RepatAlgoId>[] = [
  {
    ...
    description: "Sorts by days past clinical target length of stay. Best for clearing the most urgent LOS backlog at tertiary centers.",
  },
  {
    id: "home_bed_ready",
    label: "Bed-confirmed first",
    description: "Surfaces patients whose home hospital already has a confirmed bed available in the required unit. Best for completing transfers that are logistically ready now.",
  },
  {
    id: "closest_home",
    label: "Shortest transfer distance",
    description: "Prioritizes patients whose home hospital is geographically closest to the current tertiary center. Best for minimizing transport burden.",
  },
  {
    id: "combined",
    label: "Combined priority score",
    description: "Weighs days overdue, home bed availability, and transfer distance together. Best balanced queue management when multiple patients are ready.",
  },
];
...
function AlgoSelector<T extends string>({
  algos,
  value,
  onChange,
}: {
  ...
  return (
    <div className="mb-5 p-3 bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-xl">
      <div className="flex items-center gap-2 mb-2">
        <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Ranking algorithm</span>
      </div>
      <select
        value={value}
        onChange={(e) => onChange(e.target.value as T)}
        className="w-full text-sm font-medium px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 mb-2"
      >
        {algos.map((a) => (
  ...
  return (
    <div className="bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl p-4 mt-3.5">
      ...
      <div className="mt-2.5 text-xs text-slate-500">
        ...
        {h.transfer_center && <span className="ml-2 text-[#185FA5] font-medium">· Transfer center active</span>}
      </div>
    </div>
  );
}
  ...
  return candidates
    .map((h): ScoredHospital => {
      ...
      if (algo === "balanced") {
        score += bedPct * 40;
        if (hasSpec) score += 35;
        score += h.type === "tertiary" ? 15 : h.type === "regional" ? 10 : 0;
        if (h.transfer_center) score += 10;
      } else if (algo === "specialty_first") {
        if (hasSpec) score += 60;
        score += bedPct * 25;
        score += h.type === "tertiary" ? 10 : h.type === "regional" ? 5 : 0;
        if (h.transfer_center) score += 5;
      } else if (algo === "closest") {
        // Invert drive time: 0 min → 100 pts, 180+ min → 0 pts
        score += Math.max(0, 100 - (driveMin / 180) * 100) * 0.7;
        if (bedAvail > 0) score += 20;
        if (hasSpec) score += 10;
      } else if (algo === "capacity_first") {
        score += bedPct * 70;
        if (hasSpec) score += 20;
        if (h.transfer_center) score += 10;
      } else if (algo === "conserve_tertiary") {
        // Penalize tertiary, reward regional/critical
        score += h.type === "critical" ? 30 : h.type === "regional" ? 20 : 0;
        score += bedPct * 40;
        if (hasSpec) score += 20;
        if (h.transfer_center) score += 10;
      }
  ...
  return (
    <div>
      ...
      <div className="text-[11px] font-semibold uppercase tracking-widest text-slate-400 mb-3">
        Find the best receiving hospital
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <select value={acuity} onChange={(e) => setAcuity(e.target.value as BedKey)}
            className="text-sm px-2.5 py-1.5 border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100">
      ...
      {results === null ? null : results.length === 0 ? (
        <div className="text-center py-8 text-sm text-slate-400">
          ...
          <br />Consider out-of-state transfer or air transport to a tertiary center.
        </div>
      ) : (
        <div>
          <div className="flex items-center justify-between mb-2.5">
            <div className="text-[11px] font-semibold uppercase tracking-widest text-slate-400">
              Recommended receiving hospitals
            </div>
            <div className="text-[11px] text-slate-400 italic">
              Ranked by: <span className="font-semibold text-slate-600 dark:text-slate-300">{currentAlgo.label}</span>
            </div>
          </div>
...
function scoreRepat(
  patients: typeof REPAT_PATIENTS,
  algo: RepatAlgoId
): typeof REPAT_PATIENTS {
  const withScore = patients.map((p) => {
    ...
    let score = 0;
    if (algo === "days_over") {
      score = p.days_over * 10;
    } else if (algo === "home_bed_ready") {
      score = bedAvail > 0 ? 100 + p.days_over : 0;
    } else if (algo === "closest_home") {
      score = Math.max(0, 200 - driveMin);
    } else if (algo === "combined") {
      score = (p.days_over * 20) + (bedAvail > 0 ? 40 : 0) + Math.max(0, 40 - driveMin / 5);
    }
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
        ...
        <div className="text-[11px] text-slate-400 italic">
          Sorted by: <span className="font-semibold text-slate-600 dark:text-slate-300">{currentAlgo.label}</span>
        </div>
      </div>
            ...
            <div key={p.id} className="flex items-center gap-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3.5 py-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-slate-400 mt-0.5">
                  At <span className="font-medium text-slate-600 dark:text-slate-300">{p.at}</span> → home:{" "}
                ...
                <div className="text-[11px] text-slate-400 mt-1">
                  ...
                  <span className="ml-2">~{driveMin} min drive</span>
                </div>
                <div className="text-[11px] text-slate-400 mt-0.5">
                  {p.blocker} · {p.home} {p.acuity} beds:{" "}
                  <span className={`font-semibold ${canRepat ? "text-[#3B6D11]" : "text-[#A32D2D]"}`}>
                    {bedAvail} avail
                  </span>
                </div>
              </div>
...
function TickerBar() {
  ...
  return (
    <div className="flex gap-2 flex-wrap pb-3.5 border-b border-slate-200 dark:border-slate-700">
      {chips.map((c) => (
        <div key={c.short}
          ...
          {c.short}: <span className="font-semibold text-slate-800 dark:text-slate-100">{c.avail}</span> avail
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div className="w-full font-sans text-slate-800 dark:text-slate-100 flex flex-col pb-20">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-6 shadow-sm">
        ...
        {activeTab === "transfer" && (
          <TransferRouting algo={transferAlgo} onAlgoChange={setTransferAlgo} />
        )}
        {activeTab === "repat" && (
          <RepatriationQueue algo={repatAlgo} onAlgoChange={setRepatAlgo} />
        )}
      </div>
    </div>
  );
