...
function ScenarioBuilder({ selected, onToggle, onSelectAll, onClearAll, onViewRoadmap }: {
  selected: Set<string>;
...
function GeographicMap({ selectedRecs }: { selectedRecs: Set<string> }) {
...
const Footer = () => (
  <div className="mt-12 p-6 bg-slate-50 border border-slate-200 rounded-xl">
    ...
    <p className="text-xs text-slate-500 leading-relaxed">
      This simulation tool uses a combination of data from the Oliver Wyman "Act 167 Community Engagement:
      Recommendations" report (August 2024), publicly available Vermont hospital financial data (GMCB), Vermont
      Census/ACS data, and <strong>synthetic projections</strong> where actual data is unavailable. Financial
      projections, population data, and outcome estimates are illustrative and intended for scenario planning
      purposes only. They should not be used as the sole basis for policy decisions. All dollar amounts are in
      nominal USD. Hospital-specific data should be validated against actual GMCB filings and hospital financial
      statements before use in formal analyses.
    </p>
...
export default function Act167SimulatorPage() {
  ...
  const tabs = [
    ...
    {
      id: "hospital",
      label: "Hospital Simulator",
      icon: <BuildingOffice2Icon className="w-5 h-5" />,
      content: <><HospitalDeepDive selectedRecs={selectedRecs} /><Footer /></>,
    },
    {
      id: "financial",
      label: "Financial Modeling",
      icon: <CurrencyDollarIcon className="w-5 h-5" />,
      content: <><FinancialModeling selectedRecs={selectedRecs} /><Footer /></>,
    },
    {
      id: "equity",
      label: "Equity & Access",
      icon: <ScaleIcon className="w-5 h-5" />,
      content: <><EquityAnalysis selectedRecs={selectedRecs} /><Footer /></>,
    },
    {
      id: "map",
      label: "Geographic Map",
      icon: <MapIcon className="w-5 h-5" />,
      content: <><GeographicMap selectedRecs={selectedRecs} /><Footer /></>,
    },
    {
      id: "technology",
      label: "Technology Roadmap",
      icon: <CpuChipIcon className="w-5 h-5" />,
      content: <><TechnologyRoadmap /><Footer /></>,
    },
    {
      id: "workforce",
      label: "Workforce Planning",
      icon: <UsersIcon className="w-5 h-5" />,
      content: <><WorkforcePlanning /><Footer /></>,
    },
    {
      id: "benchmarks",
      label: "State Benchmarks",
      icon: <GlobeAltIcon className="w-5 h-5" />,
      content: <><StateBenchmarks /><Footer /></>,
    },
    {
      id: "roadmap",
      label: "Implementation Plan",
      icon: <ClipboardDocumentListIcon className="w-5 h-5" />,
      content: <><ImplementationRoadmap selectedRecs={selectedRecs} /><Footer /></>,
    },
  ];
