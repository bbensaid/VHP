...
// Fetch webinars sorted by date (soonest first)
...
export default async function WebinarsPage() {
  ...
  // Logic: First item is "Featured" (Next Up), rest are the grid
  ...
  return (
    <div className="bg-white min-h-screen pb-20">
       ...
       <div className="container mx-auto px-4 md:px-8 -mt-8">
         ...
         {/* FEATURED EVENT (The soonest one) */}
         {featured ? (
            <div className="bg-white p-8 rounded-xl border border-gray-200 shadow-xl mb-16 flex flex-col md:flex-row items-center gap-8">
                <div className="md:w-2/3">
                    <div className="flex items-center gap-3 mb-4">
                        <span className="bg-red-100 text-red-700 px-3 py-1 rounded-full text-xs font-bold uppercase inline-block">
                            Next Up
                        </span>
                    ...
                    <div className="flex gap-4">
                        <button className="px-6 py-3 bg-indigo-600 text-white font-bold rounded hover:bg-indigo-700 shadow-md transition-all">
                            Register Free
                        </button>
                        <button className="px-6 py-3 border border-gray-300 font-bold rounded hover:bg-gray-50 text-gray-700">
                            Add to Calendar
                        </button>
                    </div>
                </div>
                {/* Fallback visual for the featured event */}
                <div className="md:w-1/3 bg-slate-900 h-48 w-full rounded-lg flex items-center justify-center text-white/20 font-bold text-4xl border border-slate-700">
                    HTR
                </div>
            </div>
         ) : (
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {upcoming.map((webinar: any) => (
                        <AcademyCard 
                            ...
                            meta={formatDate(webinar.date)}
                            price="Free" // Webinars are typically free lead magnets
                            href={`/education/webinars/${webinar.slug}`} // Future-proofing: You can create a detail page later if needed
                        />
                    ))}
                </div>
            </>
