...
export default async function CaseStudiesPage() {
  ...
  // Helper for colors based on pillar (matching your globals)
      ...
      <div className="bg-slate-50 border-b border-gray-200 py-16">
        <div className="container mx-auto px-4 md:px-8 text-center max-w-3xl">
          ...
          <p className="text-xl text-gray-600">
            Real-world examples of how we apply policy, economics, and technology to solve systemic problems.
          </p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {cases.map((study: any) => (
            <div key={study._id} className="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden hover:shadow-xl transition-shadow flex flex-col">
              <div className="h-2 bg-gray-900 w-full"></div> {/* Decorative Top Bar */}
              <div className="p-8 flex-grow">
                <div className="flex justify-between items-start mb-4">
                  <span className={`px-3 py-1 rounded text-xs font-bold uppercase tracking-wide ${getPillarColor(study.pillar)}`}>
                    {study.pillar}
                  </span>
                ...
                <h3 className="text-2xl font-bold text-gray-900 mb-4">
                  <Link href={`/education/case-studies/${study.slug}`} className="hover:text-indigo-600 transition-colors">
                    {study.title}
                  </Link>
                </h3>
                ...
                {study.metrics && (
                  <div className="grid grid-cols-2 gap-4 border-t border-gray-100 pt-6">
                    {study.metrics.map((metric: string, i: number) => (
                      <div key={i}>
                        <span className="block text-lg font-black text-gray-900">{metric}</span>
                      </div>
                    ))}
                  </div>
                )}
              ...
              <div className="bg-gray-50 p-4 border-t border-gray-100 text-right">
                <Link href={`/education/case-studies/${study.slug}`} className="text-sm font-bold text-indigo-600 hover:underline">
