          ...
          <header>
            ...
            <div className="mt-6 flex flex-col md:flex-row justify-between items-start">
              <div>
                ...
                <h1 className="text-4xl md:text-5xl font-black text-slate-900">HTR Multimedia</h1>
                <p className="text-slate-500 mt-2 text-lg">Podcasts, Video Briefings, and the Full Media Library</p>
              </div>
            </div>
          </header>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8 pb-24">
        <div className="border-b border-slate-300 overflow-x-auto hide-scrollbar">
          <nav className="-mb-px flex space-x-2 min-w-max" aria-label="Tabs">
            <TabButton isActive={activeTab === 'podcasts'} onClick={() => setActiveTab('podcasts')} icon={<MicrophoneIcon className="w-5 h-5"/>} label="HTR Podcast Network" />
            <TabButton isActive={activeTab === 'videos'} onClick={() => setActiveTab('videos')} icon={<FilmIcon className="w-5 h-5"/>} label="Video Briefings" />
            <TabButton isActive={activeTab === 'library'} onClick={() => setActiveTab('library')} icon={<RectangleStackIcon className="w-5 h-5"/>} label="Full Multimedia Library" />
          </nav>
        </div>
