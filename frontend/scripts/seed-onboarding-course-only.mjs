// scripts/seed-onboarding-course-only.mjs
// Scoped copy of seed-courses.mjs that seeds ONLY course_seed.json
// (course_htr_onboarding_v1) — deliberately does NOT touch courses_tier1.json
// or any other course, per the project's "one course/lesson at a time, never
// batch" rule. Upsert-only (matched by slug), no deletes.
//
// Run with: node scripts/seed-onboarding-course-only.mjs
...
seed(courseData).catch(err => { console.error(err); process.exit(1); });
