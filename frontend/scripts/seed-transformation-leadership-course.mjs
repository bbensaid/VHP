// scripts/seed-transformation-leadership-course.mjs
//
// Seeds ONLY the "Transformation Leadership" course
// (frontend/content/course_transformation_leadership.json) into Supabase.
//
// This is the book's Chapter 12 companion course — see
// memory/project_tool_extension_queue_2026-09-21 and the 2026-09-22 build
// brief: Ch12 originally claimed a "transformation leadership" Academy
// course that did not exist; this builds it for real.
//
// Deliberately scoped to this one course. Upsert-only, no deletes, safe to
// re-run (matches on slug / composite keys) — same pattern as
// seed-five-pillars-course.mjs, generalized to read courses.pillar/level/
// chapter_ref/is_featured, which that script's source JSON didn't carry.
//
//   cd frontend && node scripts/seed-transformation-leadership-course.mjs
//
...
const course = JSON.parse(
  readFileSync(join(__dir, "../content/course_transformation_leadership.json"), "utf8"),
);
...
async function seed() {
  ...
  console.log(`  ${course.tracks.length} track(s)\n`);
  ...
  console.log(
    `✅ Done — ${nTracks} track(s), ${nLessons} lessons, ${nQuizzes} quizzes.\n` +
      `   Next: lesson bodies live in Sanity (frontend/content/_lessons_transformation_leadership.py).\n` +
      `   Then link them:\n` +
