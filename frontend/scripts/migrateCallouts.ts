...
async function migrate() {
  console.log('Starting migration: callout -> quote...')
  ...
  for (const { type, field } of TYPES_TO_CHECK) {
    ...
    for (const doc of docs) {
      ...
      const newBlocks = blocks.map((block: any) => {
        // Check if it's a block and has the style 'callout'
        if (block._type === 'block' && block.style === 'callout') {
          console.log(`Found callout in document: ${doc._id} (${type})`)
          hasChanges = true
