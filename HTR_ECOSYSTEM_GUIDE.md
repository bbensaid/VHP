...
- F–I. FAQ, Book↔Tool cross-reference, Security posture, Vermont program glossary
- J. Document Maintenance
- **K. Multi-Domain Access, Beta Testing & Feedback (2026-07 update)** — new
...
1. **Beta gate.** Any visitor without a valid beta session is redirected to `/beta` (an access-code wall). The gate is enforced in `frontend/app/layout.tsx`, not the proxy: it reads the `htr_beta` cookie and grants access only when the cookie value matches the current host. Exemptions: `/beta`, `/api/*`, `/studio`, and static assets. As of the **2026-07 update** the gate is **domain-scoped** — a code is only valid on the domains listed in its `allowed_domains`, and the granted cookie is bound to the host so it cannot be reused across the four domains. See **Appendix K** for the full model. ⚠️ **Still unconditional** — there is no env switch to disable it; for a public launch it must be removed or made toggleable (see §38).
...
Logged-in users hitting `/login` or `/signup` are redirected to `/account`; unauthenticated users hitting a protected route are sent to `/login?from=…`; under-privileged users are sent to `/upgrade?from=…`. As of the **2026-07 update**, `/login` always offers a **"Skip — continue without an account"** option (access-code holders don't need an account to browse); skip lands on the intended page unless it is truly account-gated (`/account`, `/admin`, `/studio`, `/upgrade`), in which case it lands on home. The login page is also **brand-aware** — its wordmark reads "Health Transformation Review" or "…Solutions" per the host.
...
| Beta / testing | `/beta` (access-code wall), `/tester` (Beta Tester Hub — reachable via the persistent "🧪 Tester Hub" floating button on every page) |
| Admin | `/admin`, `/admin/{users,access-codes,tester-feedback,analytics,revenue,role-changes,ingest}` |
| CMS | `/studio` |
| Key APIs | `/api/chat`, `/api/search`, `/api/stripe/{checkout,portal,webhook}`, `/api/academy/certificates`, `/api/wire`, `/api/health`, `/api/beta/verify` (domain-scoped code check), `/api/beta/clear`, `/api/admin/beta-codes`, `/api/tester-report` (saves feedback to DB + emails) |
...
*What is the difference between HTR and HTS?* Health Transformation Review (HTR) is the knowledge/analytical brand; Health Transformation Solutions (HTS) is the advisory-services brand. One deployment serves **four domains** — `healthtransformationreview.org`/`.com` (brand = `review`) and `healthtransformationsolutions.org`/`.com` (brand = `solutions`) — and resolves the brand per request from the Host header (`resolveBrand()` in `frontend/lib/brand.ts`). The only content difference is that **Advisory Services is shown on Solutions and hidden on Review**; ~99% of the app is identical. See **Appendix K** for details.
